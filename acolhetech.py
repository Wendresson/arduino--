...
frases_positivas = [
    ...
    "🌿 'Respirar fundo é o primeiro passo para reorganizar o mundo ao seu"
    " redor.'",
    ...
    "🛡️ 'Pedir ajuda não é sinal de fraqueza, mas sim de grande coragem e"
    " autoconhecimento.'",
  ...
  if imagem_encontrada:
    ...
    with col2:
      ...
      try:
        ...
        st.markdown(
            f'<div class="{classe_div}"><img'
            f' src="data:image/png;base64,{data}" width="160"></div>',
            unsafe_allow_html=True,
        )
      except Exception:
...
if st.session_state.crise:
  ...
  st.write(
      "Percebi que você está passando por um momento muito difícil. Você não"
      " está sozinho(a)! Por favor, procure **AGORA** mesmo um professor, a"
      " coordenação ou a psicologia da escola."
  )
...
if st.session_state.passo == 0:
  ...
  st.write(
      "Olá! Eu sou o **AcolheTech**. Estou aqui para conversar, te ouvir e te"
      " ajudar a organizar seus pensamentos hoje."
  )
...
# ETAPA 2: MENSAGEM INSPIRADORA
elif st.session_state.passo == 1:
  st.subheader(
      f"✨ Etapa 2/15: Uma mensagem para você, {st.session_state.nome}"
  )
...
# ETAPA 3: AVALIAÇÃO DE ENERGIA/HUMOR
  ...
  st.write("Como está sua energia para enfrentar o dia de hoje?")
  col1, col2 = st.columns(2)
  if col1.button("🔋 100% - Cheio(a) de energia"):
    st.session_state.passo = 3
    st.rerun()
  if col2.button("⚡ 50% - Cansado(a), mas levando"):
    st.session_state.passo = 3
    st.rerun()
  if col1.button("🪫 20% - Perto de esgotar"):
    st.session_state.passo = 3
    st.rerun()
  if col2.button("⚠️ 0% - Totalmente sobrecarregado(a)"):
    st.session_state.passo = 3
    st.rerun()
...
# ETAPA 6: IDENTIFICAÇÃO DA ORIGEM
  ...
  col1, col2 = st.columns(2)
  if col1.button("📚 Desafios Acadêmicos / Provas"):
    st.session_state.passo = 6
    st.rerun()
  if col2.button("🤝 Relações com Colegas / Amigos"):
    st.session_state.passo = 6
    st.rerun()
  if col1.button("🏠 Questões Familiares ou Pessoais"):
    st.session_state.passo = 6
    st.rerun()
  if col2.button("❓ Não sei explicar, apenas sinto"):
    st.session_state.passo = 6
    st.rerun()
...
# ETAPA 7: SEGUNDA PÍLULA DE INSPIRAÇÃO
  ...
  st.success(
      "🌱 **Lembrete de Autocompaixão:**\n\n'Você não precisa ter o controle"
      " de tudo o tempo todo. Fazer o seu melhor com o que você tem hoje já é o"
      " suficiente.'"
  )
...
# ETAPA 8: TEMPO PARA REFLETIR E TEMPORIZADOR
  ...
  st.write(
      "Vamos fazer uma pausa consciente de 10 segundos antes do exercício de"
      " respiração."
  )
...
# ETAPA 9: EXERCÍCIO DE RESPIRAÇÃO COM MASCOTE ANIMADO
  ...
  st.write(
      "Observe o **AcolheTech** abaixo: ele encolhe e expande no ritmo da sua"
      " respiração."
  )
  ...
  if st.button("Começar Exercício de Respiração"):
    ...
    for c in range(1, 6):
      ...
      time.sleep(1.5)
      container_msg.success(
          f"🧘 **Ciclo {c}/5:** EXPIRA suavemente pela boca..."
      )
      bar.progress(c * 20)
      time.sleep(2.5)
...
# ETAPA 11: ORIENTAÇÃO SOCIOEMOCIONAL DO ACOLHETECH
  ...
  st.info(
      "💡 **A REGRA DOS 5 MINUTOS:**\n\nQuando uma tarefa escolar ou problema"
      " parecer grande e assustador, comprometa-se a mexer nele por apenas 5"
      " minutos. Se quiser parar depois, pare. A maior barreira é sempre"
      " começar!"
  )
...
# ETAPA 13: MENSAGEM FINAL DE INSPIRAÇÃO
  ...
  st.success(
      "💙 'Você é capaz, inteligente e resistente. Lembre-se de comemorar as"
      " pequenas vitórias do seu dia!'"
  )
...
# ETAPA 14: AVALIAÇÃO DE 1 A 5
  ...
  st.write(
      f"**{st.session_state.nome}**, chegamos à penúltima etapa do nosso"
      " acolhimento."
  )
  nota = st.slider(
      "De 1 a 5, quanto esta conversa te ajudou a desacelerar?", 1, 5, 5
  )
...
# ETAPA 15: ENCERRAMENTO E REINÍCIO
  ...
  st.write("---")
  st.warning(
      "⚠️ **AVISO IMPORTANTE:** O AcolheTech é uma ferramenta de apoio e"
      " autorregulação e **não substitui** o acompanhamento profissional de"
      " psicólogos ou psiquiatras. Procure a orientação da escola sempre que"
      " precisar!"
  )
